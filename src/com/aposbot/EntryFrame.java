...
public final class EntryFrame extends Frame {
				...
				if (authFrame == null) {
					...
					authFrame.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							...
							try {
								out = new FileOutputStream("." + File.separator + "Accounts" + File.separator + u + ".properties");
								p.store(out, null);
	...
	private static void loadUsername(IClientInit init, String name) {
		...
		try {
			stream = new FileInputStream("." + File.separator + "Accounts" + File.separator + name + ".properties");
			p.load(stream);
